...
class AptosAdapterTest {
    ...
    void testInvokeSmartContract() throws ExecutionException, InterruptedException {
        ...
        List<ImmutablePair<String, String>> signatures = new ArrayList<ImmutablePair<String, String>>();

        List<String> typeArguments = new ArrayList<>();
        List<String> signers = new ArrayList<>();
        long minimumNumberOfSigners = 0;
        Transaction result =
                aptosAdapter.invokeSmartContract(functionId, methodName, typeArguments, parameters, outputParameters,
                        0, 0L, "", "", signers, signatures,
                        minimumNumberOfSigners).get();
    ...
    void testInvokeSmartContract2() throws ExecutionException, InterruptedException, JsonProcessingException {
        ...
        Transaction result =
                aptosAdapter.invokeSmartContract(functionId, methodName, typeArguments, parameters, outputParameters,
                        0, 0L, "", "",
                        signers, signatures, minimumNumberOfSigners).get();
        assert result.getState() == TransactionState.CONFIRMED;
    }

    @Test
    void testInvokeError() throws ExecutionException, InterruptedException {

        String functionId = "9f709239a4caf988527df46b7dca3797b740e408e48aa713e79a87fe85a53c4d/message";
        String methodName = "set_message2";
